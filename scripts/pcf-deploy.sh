...
bosh create-env bosh.yml --state=state.json --vars-store=creds.yml -o gcp/cpi.yml -o gcp/bosh-lite-vm-type.yml -o external-ip-not-recommended.yml -o bosh-lite.yml -o bosh-lite-runc.yml -v director_name=bosh-lite -v internal_cidr=$CIDR_RANGE -v internal_gw=$SUBNET_GATEWAY -v internal_ip=$INTERNAL_IP --var-file gcp_credentials_json=$TF_VAR_credentials_path -v project_id=$TF_VAR_project_id -v external_ip=$GLOBAL_IP -v zone=$TF_VAR_zone_id -v tags=[https-server] -v network=$NETWORK_NAME -v subnetwork=$SUBNET_NAME
